...
export default function Inventory() {
  ...
  const handleSubmit = () => {
    ...
    addPurchase(selectedFlowerId, qty, price);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-ink/60 mt-1">查看库存状态、登记新进货</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-rose-50/50">
                ...
                <th className="text-right px-5 py-3 text-sm font-medium text-ink/70">平均进价</th>
                  ...
                  <tr key={flower.id} className="hover:bg-rose-50/30 transition-colors">
                    ...
                    <td className="text-right px-5 py-4">
                      <span className={`font-serif text-lg font-bold ${
                        isOut ? "text-ink/30" : isLow ? "text-red-500" : "text-ink"
                      }`}>
                        {flower.currentStock}
